<template>
    <div v-if="activeProducts.length > 0">
        <ul class="mt-5">
            <li v-for="(activeProduct, index) in activeProducts" :key="index"
                class="flex justify-between border-2 border-green-500 gap-x-6 my-3 py-5 card-shadow bg-gray-200 rounded-2xl px-5">
                <div class="flex min-w-0 gap-x-4 ">
                    <img class="w-36 h-36 flex-none rounded-3xl bg-gray-50" :src="activeProduct.productPhotos[0].url"
                        alt="" />
                    <div class="min-w-0 flex-auto">
                        <p class="text-md font-semibold leading-6 text-gray-900">{{ activeProduct.name }}
                        </p>
                        <p class="mt-1 truncate text-xs leading-5 text-gray-500">{{ activeProduct.description }}
                        </p>
                    </div>
                </div>
                <div class="hidden shrink-0 sm:flex flex-col justify-center items-end space-y-1">
                    <button @click="makePassiveProduct(activeProduct)"
                    ...
                    <button @click="makeAwaitProduct(activeProduct)"
                    ...
                    <Menu as="div" class="relative ml-3">
                        ...
                        <transition enter-active-class="transition ease-out duration-100"
                            ...
                            <MenuItems
                                ...
                                <button @click="makePassiveProduct(activeProduct)" class="text-start w-full"
                                    :class="[active ? 'bg-gray-300 ' : '', 'block px-4 py-2 text-sm text-gray-700']">Passive</button>
                                </MenuItem>
                                <MenuItem v-slot="{ active }">
                                <button @click="makeAwaitProduct(activeProduct)" class="text-start w-full"
                                    :class="[active ? 'bg-gray-300' : '', 'block px-4 py-2 text-sm text-gray-700']">Await</button>
                                </MenuItem>
                            </MenuItems>
                        </transition>
                    </Menu>
    ...
    <div v-else class="flex justify-center items-center">
        <h2 class="font-bold text-2xl">There is no active product...</h2>
    </div>
</template>
...
export default {
    ...
    created() {
        ...
        this.getActiveProductsByCompanyId(this.getActiveUserId)
        setTimeout(() => {
    ...
    methods: {
        ...
        makePassiveProduct(product) {
            ...
            normalProductService.update(updatedProduct).then(response => {
                this.activeProducts = this.activeProducts.filter(passiveProduct => passiveProduct.id != updatedProduct.id)
            })
        },
